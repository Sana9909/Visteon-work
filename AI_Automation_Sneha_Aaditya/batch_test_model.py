...
def compute_max_new_tokens(line_len: int) -> int:
    ...
    """
    # Return a larger safe default to avoid JSON truncation with Llama 3.2 3B
    return 700
...
def main():
    ...
    for batch_start in range(0, len(sorted_lines), BATCH_SIZE):
        ...
        for i, (raw_log_line, response_text) in enumerate(zip(batch, raw_responses)):
            ...
            # Handle both "type" and "transmissionDirection" (which Llama 3.2 3B sometimes uses)
            msg_type = parsed.get("type", "")
            if not msg_type and "transmissionDirection" in parsed:
                msg_type = parsed["transmissionDirection"].lower()
            
            # Normalize "timestamp" to "time" if needed
            if "time" not in parsed and "timestamp" in parsed:
                parsed["time"] = parsed["timestamp"]

            if msg_type == "ignore" or parsed.get("error") == "not_someip":
                total_rejected += 1
                continue

            elif msg_type in ("rx", "tx"):
                total_parsed += 1
                # Make sure type is consistently set for sorting/processing later
