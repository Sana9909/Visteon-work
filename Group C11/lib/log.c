                           ...
                           const FaultStatus *faults)
{
    if ((input != NULL) && (status != NULL) && (faults != NULL)) {
        ...
        /* Deferred Clamping/Correction Reporting (Out-of-band to profiling) */
        if ((status->clamping_flags & CLAMP_SPEED) != 0U) {
            sys_log_clamping("INPUT", "speed", (int32_t)status->invalid_speed, (int32_t)status->last_valid_speed);
        }
        if ((status->clamping_flags & CLAMP_TEMP) != 0U) {
            sys_log_clamping("INPUT", "temperature", (int32_t)status->invalid_temperature, (int32_t)status->last_valid_temperature);
        }
        if ((status->clamping_flags & CLAMP_GEAR) != 0U) {
            sys_log_clamping("INPUT", "gear", (int32_t)status->invalid_gear, (int32_t)status->last_valid_gear);
        }
        if ((status->clamping_flags & CLAMP_MODE) != 0U) {
            sys_log_clamping("INPUT", "mode", 4, (int32_t)status->last_valid_mode);
        }
                       ...
                       sys_file_t logfile)
{
    ...
    /* Mirror to console if logfile is a real file */
    if (logfile != NULL) {
        write_summary(NULL, input, status, faults);
    }
}
...
void log_cycle_timing(sys_file_t logfile, uint16_t cycle_num, const CycleTiming *timing)
{
    if (timing != NULL) {
        ...
        /* Mirror to console */
        sys_printf("\n  [PERF] Cycle %u  CPU Cycles (excluding printf):\n", (unsigned int)cycle_num);
        sys_printf("    read_inputs           = %llu\n", (unsigned long long)timing->read_inputs);
        sys_printf("    validate_inputs       = %llu\n", (unsigned long long)timing->validate_inputs);
        sys_printf("    update_mode           = %llu\n", (unsigned long long)timing->update_mode);
        sys_printf("    run_control_checks    = %llu\n", (unsigned long long)timing->run_control_checks);
        sys_printf("    update_fault_status   = %llu\n", (unsigned long long)timing->update_fault_status);
        sys_printf("    evaluate_system_state = %llu\n", (unsigned long long)timing->evaluate_system_state);
        sys_printf("    TOTAL                 = %llu\n", (unsigned long long)timing->total);
    }
}
