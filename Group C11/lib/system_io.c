...
static sys_file_t g_sim_log = NULL;
...
void sys_log_clamping(const char *module, const char *param, int32_t val, int32_t fallback)
{
    ...
    if (g_suppress_io == 0) {
        (void)printf("[%s] Invalid %s %d -> using last valid %d\n",
                     module, param, (int)val, (int)fallback);
    }
    
    if (g_sim_log != NULL) {
        (void)fprintf((FILE*)g_sim_log, "  [ERROR] %s: Invalid %s (%d) clamped to %d\n",
                      module, param, (int)val, (int)fallback);
    }
}
