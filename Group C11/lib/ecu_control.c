...
uint16_t get_total_tests(void) {
    load_tests_from_json(); /* Ensure tests are loaded */
    return g_num_loaded_tests;
}

const char* get_test_name_by_index(uint16_t index) {
    if (index < g_num_loaded_tests) {
        return g_tests[index].name;
    }
    return "INVALID TEST";
}

void run_all_test_cases(int16_t target_index, VehicleStatus *status, FaultStatus *faults)
{
    if ((status != NULL) && (faults != NULL)) {
        ...
        if ((logfile != NULL) && (csv_report != NULL)) {
            ...
            sys_set_simulation_log(logfile);
            ...
            for (uint32_t iteration = 0; iteration < NUM_ITERATIONS; iteration++) {
                ...
                for (t = 0U; t < g_num_loaded_tests; t++) {
                    /* Skip test if filter is active and this isn't the target index */
                    if ((target_index >= 0) && ((uint16_t)target_index != t)) {
                        continue;
                    }
                ...
                for (c = 0U; c < test->num_cycles; c++) {
                    ...
                    if (test->reset_triggered[c] != 0) {
                        ...
                        sys_printf("  [SYSTEM] Manual Reset Triggered\n");
                    }
                    ...
                    /* Performance Fast Path: triggers for any stable input cycle */
                    int stable_cycle = (!inputs_changed && (c > 0U));
                    ...
                    if (stable_cycle) {
                        ...
                        if (do_logging) {
                            sys_fprintf(logfile, "  [SEQ] Step 1: read_inputs...    [STABLE]\n");
                            sys_printf("  [SEQ] Step 1: read_inputs...    [STABLE]\n");
                        }
                        ...
                        if (do_logging) {
                            sys_fprintf(logfile, "  [SEQ] Step 2: validate_inputs... [STABLE]\n");
                            sys_printf("  [SEQ] Step 2: validate_inputs... [STABLE]\n");
                        }
                        tsc0 = read_tsc(); tsc1 = read_tsc();
                        ...
                        if (do_logging) {
                            sys_fprintf(logfile, "  [SEQ] Step 3: update_mode....... [FAULT LOCKED]\n");
                            sys_printf("  [SEQ] Step 3: update_mode....... [FAULT LOCKED]\n");
                        }
                        ...
                        if (do_logging) {
                            sys_fprintf(logfile, "  [SEQ] Step 4: run_controls...... [STABLE]\n");
                            sys_printf("  [SEQ] Step 4: run_controls...... [STABLE]\n");
                        }
                        tsc0 = read_tsc(); tsc1 = read_tsc();
                        ...
                        if (do_logging) {
                            sys_fprintf(logfile, "  [SEQ] Step 5: update_faults..... [STABLE]\n");
                            sys_printf("  [SEQ] Step 5: update_faults..... [STABLE]\n");
                        }
                        tsc0 = read_tsc();
                        ...
                        if (do_logging) {
                            sys_fprintf(logfile, "  [SEQ] Step 6: evaluate_state.... [SAFE LATCHED]\n");
                            sys_printf("  [SEQ] Step 6: evaluate_state.... [SAFE LATCHED]\n");
                        }
                    ...
                    status->clamping_flags = CLAMP_NONE;
                    g_suppress_io = 1;
                    ...
                    if (do_logging) {
                        const char *lbl = inputs_changed ? "[OK]" : "[STABLE]";
                        sys_fprintf(logfile, "  [SEQ] Step 1: read_inputs...    %s\n", lbl);
                        sys_printf("  [SEQ] Step 1: read_inputs...    %s\n", lbl);
                    }
                    tsc0 = read_tsc();
                    /* Inputs are already in 'active_input' via the test definition */
                    tsc1 = read_tsc();
                    update_perf(PERF_READ_INPUTS, tsc1 - tsc0);
                    timing.read_inputs = tsc1 - tsc0;
                    ...
                    if (do_logging) {
                        const char *lbl = inputs_changed ? "[OK]" : "[STABLE]";
                        sys_fprintf(logfile, "  [SEQ] Step 2: validate_inputs... %s\n", lbl);
                        sys_printf("  [SEQ] Step 2: validate_inputs... %s\n", lbl);
                    }
                    ...
                    if (do_logging) {
                        ...
                        if (status->current_mode == MODE_FAULT) {
                            lbl = (status->previous_mode == MODE_FAULT) ? "[FAULT LOCKED]" : "[FAULT DETECTED]";
                        } else if (stable_cycle) {
                            lbl = "[STABLE]";
                        }
                        sys_fprintf(logfile, "  [SEQ] Step 3: update_mode....... %s\n", lbl);
                        sys_printf("  [SEQ] Step 3: update_mode....... %s\n", lbl);
                    }
                    tsc0 = read_tsc(); update_mode(status, &active_input, faults); tsc1 = read_tsc();
                    update_perf(PERF_UPDATE_MODE, tsc1 - tsc0);
                    timing.update_mode = tsc1 - tsc0;
                    ...
                    if (do_logging) {
                        const char *lbl = inputs_changed ? "[FAULT(S) RAISED]" : "[STABLE]";
                        sys_fprintf(logfile, "  [SEQ] Step 4: run_controls...... %s\n", lbl);
                        sys_printf("  [SEQ] Step 4: run_controls...... %s\n", lbl);
                    }
                    ...
                    if (do_logging) {
                        const char *lbl = inputs_changed ? "[OK]" : "[STABLE]";
                        sys_fprintf(logfile, "  [SEQ] Step 5: update_faults..... %s\n", lbl);
                        sys_printf("  [SEQ] Step 5: update_faults..... %s\n", lbl);
                    }
                    ...
                    if (do_logging) {
                        ...
                        if (status->system_state == STATE_SAFE) {
                            lbl = (status->previous_state == STATE_SAFE) ? "[SAFE LATCHED]" : "[SAFE STATE REACHED]";
                        } else if (stable_cycle) {
                            lbl = "[STABLE]";
                        }
                        sys_fprintf(logfile, "  [SEQ] Step 6: evaluate_state.... %s\n", lbl);
                        sys_printf("  [SEQ] Step 6: evaluate_state.... %s\n", lbl);
                    }
                    tsc0 = read_tsc(); evaluate_system_state(status, faults); tsc1 = read_tsc();
                    update_perf(PERF_EV_STATE, tsc1 - tsc0);
                    timing.evaluate_system_state = tsc1 - tsc0;
                ...
                int test_passed = (mode_match && state_match && faults_match);

                if (iteration == (NUM_ITERATIONS - 1U)) {
                    if (test_passed != 0) { pass_count++; } else { fail_count++; }

                    /* Report to log and console in uniform format */
                    if (do_logging) {
                        sys_fprintf(logfile, "\n  [RESULT] %s\n", (test_passed ? "PASS" : "FAIL"));
                        sys_printf("\n  [RESULT] %s\n", (test_passed ? "PASS" : "FAIL"));
                        sys_fprintf(logfile, "  [PERF] Test Total: %llu CPU cycles\n", (unsigned long long)test_total_cycles);
                        sys_printf("  [PERF] Test Total: %llu CPU cycles\n", (unsigned long long)test_total_cycles);
                        
                        if (test_passed == 0) {
                            sys_fprintf(logfile, "  [DETAILS] Mismatches against cumulative baseline:\n");
                            sys_printf("  [DETAILS] Mismatches against cumulative baseline:\n");
                        ...
                        /* Minimal console summary for manual/non-logging modes */
                        sys_printf("\n  TEST %s [%s]\n", test->name, (test_passed ? "PASS" : "FAIL"));
                ...
                if (do_logging) {
                    ...
                    sys_fprintf(csv_report, "\"%s\",\"%s\",\"%s\",\"Mode=%s, State=%s, Faults=%s\",\"%s\"\n",
                                ...
                                mode_to_name(status->current_mode), state_to_name(status->system_state), 
                                fault_buf, (test_passed ? "PASS" : "FAIL"));
            ...
            for (t = 0U; t < g_num_loaded_tests; t++) {
                /* Skip analysis for tests that weren't run */
                if (ranking[t].total_cycles_sum == 0U) {
                    continue;
                }
            ...
            sys_fprintf(logfile, "\n  [EFFICIENCY BEST CASE] : %s (%llu cycles/cycle)\n", ranking[best_idx].name, (ranking[best_idx].total_cycles_sum / NUM_ITERATIONS) / ((ranking[best_idx].num_cycles > 0U) ? ranking[best_idx].num_cycles : 1U));
            sys_printf("\n  [EFFICIENCY BEST CASE] : %s (%llu cycles/cycle)\n", ranking[best_idx].name, (ranking[best_idx].total_cycles_sum / NUM_ITERATIONS) / ((ranking[best_idx].num_cycles > 0U) ? ranking[best_idx].num_cycles : 1U));
            sys_fprintf(logfile, "  [EFFICIENCY WORST CASE]: %s (%llu cycles/cycle)\n", ranking[worst_idx].name, (ranking[worst_idx].total_cycles_sum / NUM_ITERATIONS) / ((ranking[worst_idx].num_cycles > 0U) ? ranking[worst_idx].num_cycles : 1U));
            sys_printf("  [EFFICIENCY WORST CASE]: %s (%llu cycles/cycle)\n", ranking[worst_idx].name, (ranking[worst_idx].total_cycles_sum / NUM_ITERATIONS) / ((ranking[worst_idx].num_cycles > 0U) ? ranking[worst_idx].num_cycles : 1U));
            sys_fprintf(logfile, "  Detailed ranking saved to: test_performance_ranking.csv\n");
